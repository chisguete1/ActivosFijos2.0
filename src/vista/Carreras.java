...
public class Carreras extends javax.swing.JPanel {   
    ...
    private void btnEliminarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnEliminarActionPerformed
        int filaSeleccionada = tbCarreras.getSelectedRow();
        if (filaSeleccionada != -1) {
            int idCarrera = (int) modeloTabla.getValueAt(filaSeleccionada, 0);
            if (carreraControlador.eliminarCarrera(idCarrera)) {
                JOptionPane.showMessageDialog(this, "Carrera eliminada con éxito");
                cargarCarreras();
                limpiarCampos();
                carreraSeleccionada = -1; // Resetear para el siguiente registro
            } else {
                JOptionPane.showMessageDialog(this, "Error al eliminar la carrera", "Error", JOptionPane.ERROR_MESSAGE);
            }
        } else {
            JOptionPane.showMessageDialog(this, "Debe seleccionar una carrera para eliminar", "Error", JOptionPane.ERROR_MESSAGE);
        }
